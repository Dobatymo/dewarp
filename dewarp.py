...
class Dewarp(WarpFrame):
    ...
    def LoadFile(self, path):
        ...
        self.magnified_bitmap = (
            self.image.Copy().Rescale(self.image.Width * self.magf, self.image.Height * self.magf).ConvertToBitmap()
        )
        self.bufferedwarped = None
        self.OnSize(None)
    ...
    def Draw(self, resize=False):
        ...
        magsizey = 100
        rect = wx.Rect(
            limit(x * self.widthfactor * self.magf - 50, 0, self.magnified_bitmap.Width - magsizex),
            limit(y * self.heighfactor * self.magf - 50, 0, self.magnified_bitmap.Height - magsizey),
            magsizex,
            magsizey,
        )
    ...
    def OnKeyDown(self, event):  # doesn't work
        logging.debug(f"OnKeyDown: {event.GetKeyCode()}")
    ...
    def OnOpen(self, event):
        with wx.FileDialog(
            self, "Open XYZ file", "", "", "Image files|*.bmp;*.png;*.jpg;*.tif", wx.FD_OPEN | wx.FD_FILE_MUST_EXIST
        ) as openFileDialog:
    ...
    def Warp(self, outwidth, outheight, flags=cv2.INTER_CUBIC):
        ...
        logging.debug(f"corners: {[str(i) for i in pointssrc]!s}, pp: {principal_point!s}")
    ...
    def Binary(self, img):
        ...
        if self.threshold_mode == ThresholdMode.Const:
            ...
            logging.info(f"Threshold value is {threshold}")
            __, img = cv2.threshold(img, threshold, 255, cv2.THRESH_BINARY)

        elif self.threshold_mode == ThresholdMode.Mean:
            img = cv2.adaptiveThreshold(
                img, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, self.blocksize, self.constant
            )

        elif self.threshold_mode == ThresholdMode.Gauss:
            img = cv2.adaptiveThreshold(
                img, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, self.blocksize, self.constant
            )
    ...
    def DoFilter(self, img):
        ...
        if self.colorratio < 100:
            ...
            logging.debug(f"ratio={ratio}, neutral={neutralcolor}")
    ...
    def OnSave(self, event):
        ...
        with wx.FileDialog(
            self, "Save file", "", defaultFile, wildcard, wx.FD_SAVE | wx.FD_OVERWRITE_PROMPT
        ) as saveFileDialog:
